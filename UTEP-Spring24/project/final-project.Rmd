...
2. Extend the features (capabilities) of the base application. Here are some suggestions for extension: 
    - Personalize the app by using your preferred application layout (Checkout the [Iris dashboard codes](https://github.com/williamagyapong/workshops/blob/main/UTEP-Spring24/irisApp/demo_app.R) we went over, and [Application Layout Guide](https://shiny.posit.co/r/articles/build/layout-guide/))
...
hourly_tweet <- obama_tweets_clean |>
  # extract the hour from each time tweet was posted
  mutate(hour = hour(with_tz(created_at, tzone="EST"))) |> 
